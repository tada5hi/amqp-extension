...
- `amqp-extension db:create` to create the database
- `amqp-extension db:drop` to drop the database
...
"scripts": {
  ...
  "db:create": "ts-node ./node_modules/amqp-extension/dist/cli/index.js db:create",
  "db:drop": "ts-node ./node_modules/amqp-extension/dist/cli/index.js db:drop",
  ...
}
```
...
import {createDatabase, dropDatabase} from "amqp-extension";
...
```typescript
import {craeteDatabase, dropDatabase, buildConnectionOptions} from "amqp-extension";
...
```typescript
// Only drop database if it does exists.
import {applyFields} from "amqp-extension";
...
```typescript
// Only drop database if it does exists.
import {applyFilters} from "amqp-extension";
...
```typescript
// Only drop database if it does exists.
import {applyIncludes} from "amqp-extension";
...
```typescript
// Only drop database if it does exists.
import {applyPagination} from "amqp-extension";
...
```typescript
// Only drop database if it does exists.
import {applySort} from "amqp-extension";
